...
export class EscolaComponent implements OnInit {
  ...
  public constructor(
    private dialog: MatDialogRef<EscolaComponent>,
  ...
  private get modelo(): EscolaModel {
    let model = Object.assign(this.modeloApi, this.formCadastroEscola.value);
  ...
  private set modelo(modelo: EscolaModel) {
    ...
    this.formCadastroEscola.patchValue({
  ...
  buildForm() {
    ...
    this.formCadastroEscola = this.formBuilder.group({
      ID_ESCOLA: [null],
      ID_TIPO_ESCOLA: [1],
      RG_MEC: [null],
      DT_CAD: [new Date()],
  ...
  public addNewContact(): void {
    this.contatos = this.formCadastroEscola.get("contatos") as FormArray;
  ...
  public onSubmit(): void {
    this.formUtil.verificaValidacoes(this.formCadastroEscola);
    if (this.formCadastroEscola.valid) this.inserirOuAtualizar();
  ...
  private inserirOuAtualizar(): void {
    ...
    if (!this.escolaData) {
      ...
      this.escolaService.inserir(this.modelo).subscribe(
        ...
        (error) => {
          this.mensagem.enviar("Falha na inclusão da escola.", false);
          console.log(error);
        }
      );
    } else {
      this.escolaService.atualizar(this.modelo).subscribe(
        ...
        (error) => {
          this.mensagem.enviar("Falha na atualização da escola", false);
          console.log(error);
        }
      );
    }
  ...
  PesquisarLogradouroGrid(): void {
    console.log("2")
    this.formUtil.verificaValidacoes(this.formCadastroEscola);
    if (this.formCadastroEscola.value.RG_MEC != null) {
      this.escolaService
        .PesquisarEscolaContato(this.formCadastroEscola.value.RG_MEC)
        .subscribe((value) => {
      ...
      this.escolaService
        .PesquisarEscolaFidelidade(this.formCadastroEscola.value.RG_MEC)
        .subscribe((value) => {
          this.BuscandoEscola = false;

          if (value.Data == null) {
            this.mensagem.enviar(
              "Escola informada não existe ou não encontrada.",
              false
            );
            return;
          }
      ...
      this.addressService
        .PesquisarEscolaLogradouroGrid(this.formCadastroEscola.value.RG_MEC)
        .subscribe(
          (value) => {
  ...
  preencherDadosCliente(search: EscolaModel) {
    this.formCadastroEscola.get("ID_ESCOLA").patchValue(search.ID_ESCOLA);
    this.formCadastroEscola.get("NM_ESCOLA").patchValue(search.NM_ESCOLA);
    this.formCadastroEscola.get("DT_CAD").patchValue(search.DT_CAD);
    this.formCadastroEscola.get("OBS").patchValue(search.OBS);
  }
  ...
  PreencherDadosClienteContato(search: any) {
    if (search != null) {
      ...
      this.formCadastroEscola.get("contatos").patchValue(saida[0]);
    }
  }
