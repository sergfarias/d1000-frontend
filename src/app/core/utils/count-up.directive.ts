...
export class CountUpDirective implements OnInit {
  ...
  private createCountUp(sta, end, dec, dur) {
    if (isPlatformBrowser(this.platformId)) {
      ...
      // construct countUp
      //let _countUp = new CountUp(this.el.nativeElement, sta, end, dec, dur, this.options);
      let countUp = new CountUp(this.el.nativeElement, end, dec);
      if (end > 999) {
        // make easing smoother for large numbers
        //countUp = new CountUp(this.el.nativeElement, sta, end - 100, dec, dur / 2, this.options);
        countUp = new CountUp(this.el.nativeElement, end - 100, dec);
      } 
